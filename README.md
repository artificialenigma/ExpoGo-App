...
### Step 2: (Optional) Yarn or use `npm`

This project shows `yarn` usage in some examples. If you don't have `yarn` installed you can continue using `npm` (no global install required). If you prefer to install `yarn` globally, run:
...
Verify installation (only if you installed yarn):
...
```bash
# Using npm (no global yarn required)
npm install
...
```bash
# Using npm (preferred if you don't have yarn)
npm run start
npm run ios
npm run android
npm run web
...
## 🔐 Secrets & CI (important)

- Do NOT commit secret values to the repository. Files named `.env` and `.env.*` are now ignored by git; use `.env.example` as a template.
- For local development, create a `.env` from `.env.example` and populate secret values.
- For CI or production builds (recommended), set secrets using your CI provider's secret store (e.g., GitHub Actions secrets, CircleCI environment variables) and inject them at build time.

Example (GitHub Actions snippet) — set `API_URL` and `SECRET_KEY` in repository Settings → Secrets, then in workflow:
...
- For Expo EAS builds, prefer `eas secret:create` or the EAS secret management flow.
