        ...
        <View className="bg-white dark:bg-neutral-800 rounded-xl shadow-sm overflow-hidden mb-8">
          ...
          {impacts.length === 0 ? (
            ...
            impacts.map((impact, index) => (
              <View
                key={impact.id}
                className={`flex-row justify-between items-center p-4 border-b border-gray-100 dark:border-neutral-700 ${
                  index % 2 === 0 ? 'bg-gray-50/50 dark:bg-neutral-800' : ''
                }`}>
                ...
                <View
                  className={`px-2 py-1 rounded-full ${
                    impact.magnitude > 2.0 ? 'bg-red-100' : 'bg-orange-100'
                  }`}>
                  <Text
                    className={`text-xs font-bold ${
                      impact.magnitude > 2.0
                        ? 'text-red-700'
                        : 'text-orange-700'
                    }`}>
