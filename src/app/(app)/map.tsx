...
export default function MapScreen() {
  ...
  const onMapPress = (e: any) => {
    ...
    const validPoints = selectedSession.data.filter(
    ...
    <View className="flex-1 bg-gray-50 dark:bg-neutral-900">
      ...
      <View style={styles.mapContainer}>
        <MapView
          ...
          {/* Impact Markers */}
          {selectedSession &&
            selectedSession.impacts &&
            selectedSession.impacts.map(
              (impact, index) =>
                impact.lat &&
                impact.lon && (
                  <Marker
              ...
              <Callout>
                <View className="w-48 p-2">
                  ...
                  <Text className="text-xs font-bold text-purple-600">
                    Gyro: X:{selectedPoint.gyro.x.toFixed(2)} Y:
                    {selectedPoint.gyro.y.toFixed(2)} Z:
                    {selectedPoint.gyro.z.toFixed(2)}
                  </Text>
                  <Text className="text-xs font-bold text-red-600">
                    Accel: X:{selectedPoint.accel.x.toFixed(2)} Y:
                    {selectedPoint.accel.y.toFixed(2)} Z:
                    {selectedPoint.accel.z.toFixed(2)}
                  </Text>
                </View>
              </Callout>
            </Marker>
        ...
        {selectedSession && (
          <View className="absolute bottom-4 left-4 right-4 rounded-xl bg-white p-4 shadow-lg dark:bg-neutral-800">
            ...
            <Text className="text-sm text-gray-500 dark:text-gray-400">
              Distance:{' '}
              {calculateDistance(getRouteCoordinates(selectedSession)).toFixed(
                2,
              )}{' '}
              km
            </Text>
            ...
            sessions.map(session => (
              <TouchableOpacity
                ...
                <View className="flex-row items-center justify-between">
                  ...
                  <MapPin
                    color={
                      selectedSession?.id === session.id ? '#3b82f6' : '#9ca3af'
                    }
                    size={24}
                  />
                </View>
